...
int logic_Program::backInsSingMove(int cur_m_id,int pre_m_id) {
	...
	if( 0 == pre_m_id ) {  ////////////可能有危险
		//如果前插一棵空树
		logic_TreeNode tmpCurNode = * (mvmu_ModuleId_TreeMap[pre_m_id]->node_search(cur_m_id)) ;
...
int logic_Program::backInsMultiMove(int cur_m_id,int pre_m_id) {
	...
	/////////////开始/////////////
	logic_Tree *insTree = mvmu_ModuleId_TreeMap[pre_m_id]; //待插入树
	logic_Tree *oldTree = mvmu_ModuleId_TreeMap[cur_m_id]; //待删除树
	...
	logic_TreeNode * insNode = oldTree->node_search(cur_m_id); //待插入节点

	///// step1、删除旧树节点
	///// 注：这里必须第一步，不然 del_node_notConn 方法会删掉错误 parent 的孩子

	if( cur_m_id != oldTree->mvi_TreeID ) {

		//并非树根
		oldTree->del_node_notConn(cur_m_id); //断开 oldTree 此节点后的所有连接
